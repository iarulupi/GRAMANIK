...
let colorSubrayadoActual = "#000000";
let contadorSubrayados = 0;

function elegirColorSubrayado(color, boton) {
    colorSubrayadoActual = color;

    document
        .querySelectorAll(".color-subrayado")
        .forEach(opcion => opcion.classList.remove("activo"));
...
function subrayarSeleccion() {
    ...
    const idSubrayado = `subrayado-${++contadorSubrayados}`;
    ...
    fragmentos
        ...
        .forEach(fragmento => {
            ...
            subrayado.dataset.subrayadoId = idSubrayado;
            subrayado.dataset.subrayadoColor = colorSubrayadoActual;
...
function actualizarSubrayadoContinuo() {
    ...
    resultado
        ...
        .forEach(contenedor => {
            ...
            subrayados.forEach((subrayado, indice) => {
                const id =
                    subrayado.dataset.subrayadoId ||
                    `subrayado-antiguo-${indice}`;

                if (!grupos.has(id)) {
                    grupos.set(id, []);
                }

                grupos.get(id).push(subrayado);
            });
            ...
            grupos.forEach(grupo => {
                ...
                const color =
                    grupo[0].dataset.subrayadoColor ||
                    "#000000";
